...
export default function AttestationPage() {
  ...
  const [qMode, setQMode] = useState<QMode>('image');

  const [showCreate, setShowCreate] = useState(false);
  const [newColl, setNewColl] = useState({
    authorName: '', collectionName: '', variantCount: 1,
    year: new Date().getFullYear(), duration: 120, price: 0,
  });
  ...
  const loadTests = async () => {
    try { const r = await api.get('/tests?type=ATTESTATION'); setTests(r.data); } catch {}
  };
  ...
  const createCollection = async () => {
    ...
    try {
      ...
      setShowCreate(false);
      setNewColl({ authorName: '', collectionName: '', variantCount: 1, year: new Date().getFullYear(), duration: 120, price: 0 });
      await loadTests();
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h1 style={{ color: theme.text, fontSize: 22, fontWeight: 700 }}>📝 Atestatsiya</h1>
      ...
      {showCreate && (
        <div style={{ ...s.card, marginBottom: 20 }}>
          <h3 style={{ color: theme.text, fontWeight: 700, marginBottom: 14 }}>📚 Yangi muallif / to'plam yaratish</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div>
              ...
              <input value={newColl.collectionName} onChange={e => setNewColl(p => ({ ...p, collectionName: e.target.value }))}
                placeholder="Masalan: 2024 Atestatsiya"
                style={{ width: '100%', padding: '8px 12px', backgroundColor: theme.input, border: `1px solid ${theme.border}`, color: theme.text, borderRadius: 8, outline: 'none' }} />
            </div>
            {[
        ...
        {selectedTest ? (
          <div>
            ...
            <div style={{ ...s.card, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <input
                type="text"
                placeholder="masalan 201 yoki AT1"
                        ...
                        {[0, 1].map(col => {
                          ...
                          <div key={q.orderNo} style={{ ...s.card, borderRadius: 12, padding: '12px 16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                              ...
                              <span style={{ fontSize: 11, backgroundColor: '#10b98120', color: '#10b981', padding: '2px 8px', borderRadius: 5, fontWeight: 600 }}>A–D</span>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    {[0, 1].map(col => {
                      ...
                      <div key={q.orderNo} style={{ ...s.card, borderRadius: 12, padding: '12px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                          <span style={{ color: theme.accent, fontWeight: 800, fontSize: 15, minWidth: 28 }}>{q.orderNo}</span>
                          <span style={{ fontSize: 11, backgroundColor: '#10b98120', color: '#10b981', padding: '2px 7px', borderRadius: 5, fontWeight: 600 }}>A–D</span>
                          <div style={{ flex: 1 }} />
                          <span style={{ color: theme.text, opacity: 0.5, fontSize: 11 }}>Ball:</span>
                          <input type="number" defaultValue={q.scorePoint || 1} min={0} max={10} step={0.5}
                            onBlur={e => setScore(q.orderNo, parseFloat(e.target.value))}
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          <span style={{ color: theme.text, opacity: 0.5, fontSize: 12 }}>To'g'ri javob:</span>
                          {OPTS.map(opt => (
                            <button key={opt} onClick={() => saveAnswer(q.orderNo, opt)}
